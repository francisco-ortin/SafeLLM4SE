...
from safellm4se.sampling.models import SamplerSettings
...
def _parse_evaluator_parameters(tokens: list[str]) -> dict[str, Any]:
    ...
    Raises:
        ValueError: If a parameter is missing a value or has an empty name.
    """
    ...
    while token_index < len(tokens):
        ...
        if "=" in token:
            ...
            if next_token_index >= len(tokens):
                raise ValueError(f"Evaluator parameter {token} requires a value.")
            raw_value = tokens[next_token_index]
            ...
            raise ValueError(
                "Evaluator parameters must use name=value or --name value syntax."
            )
        clean_name: str = _normalize_parameter_name(name)
        if not clean_name:
