"""Shared statistical utilities for SafeLLM4SE sampling and reporting."""
...
def mean_confidence_interval(
    ...
    selected_method: str = select_continuous_confidence_interval_method(
        values,
        method,
    )
    if selected_method == "bootstrap":
        low_value, high_value = bootstrap_mean_confidence_interval(
            values,
            confidence_level,
            bootstrap_samples,
        )
        return low_value, high_value, selected_method

    low_value, high_value = t_mean_confidence_interval(values, confidence_level)
    return low_value, high_value, selected_method
...
def t_mean_confidence_interval(
    ...
    critical_value: float = t_critical_value(confidence_level, len(values) - 1)
    margin: float = critical_value * standard_deviation / math.sqrt(len(values))
...
def wilson_interval(
    ...
    Args:
        ...
        sample_size: Total number of binary observations.
        confidence_level: Confidence level used for interval estimation.
    ...
    if successes > sample_size:
        raise ValueError("successes cannot be greater than sample_size.")
    if sample_size == 0:
        return math.nan, math.nan
    z_score: float = normal_quantile(confidence_level)
    proportion: float = successes / sample_size
    denominator: float = 1.0 + z_score * z_score / sample_size
    center: float = (
        proportion + z_score * z_score / (2.0 * sample_size)
    ) / denominator
    margin: float = (
        z_score
        * math.sqrt(
            (
                ...
                + z_score * z_score / (4.0 * sample_size)
            )
            / sample_size
        )
        / denominator
...
def confidence_interval(
    ...
    if metric_type == "binary":
        ...
        low_value, high_value = wilson_interval(
            successes,
            len(values),
            confidence_level,
        )
        return low_value, high_value, "wilson"
